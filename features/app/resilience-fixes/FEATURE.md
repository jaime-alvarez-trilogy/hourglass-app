...
| 2026-05-28 | [05-onboarding-defense](specs/05-onboarding-defense/spec.md) | Complete. `NotContributorError(avatarTypes)` added to `src/api/errors.ts` (FR1). `src/api/auth.ts` rewrites `extractConfigFromDetail` to return `null` when `/detail` lacks a usable `assignment` (FR2), rewrites `fetchConfigFromAssignments` to read the Spring page envelope `response.content` with a bare-array fallback for shape drift (FR3), and rewires `fetchAndBuildConfig` to throw `NotContributorError(avatarTypes)` when both paths fail — **removes the userId-only last-resort branch** that was producing a broken dashboard per CROSSOVER_API §15.F7 (FR4). `useSetup` adds an `OnboardingStep` value `'not-contributor'` and a `nonContributorRoles: string[] \| null` field on `UseSetupResult`; the `_buildConfig` catch block adds a `NotContributorError` branch **before** the `AuthError` branch that sets the step, populates `nonContributorRoles`, leaves `pendingConfig`/`error` null, and calls `log.error('onboarding.not-contributor', err, {avatarTypes})` via spec 08 (FR5/FR6). New screen `app/(auth)/not-contributor.tsx` reads `nonContributorRoles` from `useOnboarding()`, renders the detected roles (or `"unknown"` when null/empty), and offers a Sign Out button that calls `clearAll()` + `invalidateAuthToken()` (spec 04 FR7 contract) + `router.replace('/(auth)/welcome')` (FR7). Route registered in `app/(auth)/_layout.tsx` with `gestureEnabled: false`. `app/(auth)/verifying.tsx` adds an `else if (step === 'not-contributor')` branch in the step-driven `useEffect` (FR8). Commits `4f4679a` (spec), `348dfc9` (red tests, +41 failing), `7ebfe02` (FR1), `69afc91` (FR2/3/4), `5685e29` (FR5/6), `655acc2` (FR7/8), `11895b0` (review fix: instanceof NetworkError), `b447355` (test strengthening). Full suite 4074/4074 green. Resolves CROSSOVER_API §15.F5 (pure-manager `/detail` schema) + §15.F6 (Spring page envelope). User-visible error wording adopted verbatim from spec-research; new error class chosen over tagged ApiError to align with the existing `errors.ts` convention. |
| 2026-05-28 | [07-notification-lifecycle](specs/07-notification-lifecycle/spec.md) | Complete. Closes the remaining notification-lifecycle defects from §8.2 / §8.3 / §8.4. **§8.3 RESOLVED**: calendar schedulers now pass deterministic `identifier`s (`hourglass:thursday`, `hourglass:monday-summary`, `hourglass:monday-expiry`) — iOS replaces same-id schedules, no AsyncStorage ID tracking, no cancel/setItem race. **§8.2 RESOLVED**: new `src/lib/scheduleLock.ts` exports `withScheduleLock<T>(fn)` — AsyncStorage-backed best-effort mutex on key `notif_schedule_lock` (stale-after-30 s). `scheduleAll` wraps its scheduler calls; `handleBackgroundPush` wraps `scheduleLocalNotification`. Lock contention on the push side skips the notification but still calls `savePrevIds` (preserves the spec-06 dedup state contract). **§8.4 MITIGATED**: same module exports `sweepOrphanNotifications`, called at every `scheduleAll` mount — iterates `getAllScheduledNotificationsAsync()` and cancels any `hourglass:*` not in `EXPECTED_IDENTIFIERS`, plus `multiRemove`s legacy `notif_*_id` keys. `inFlightRef` from spec 01 (`bebfb0f`) preserved as intra-hook defense-in-depth. `clearAll` (FR7) extended with `notif_expiry_id` (was missing from spec-06 wipe list) and `notif_schedule_lock`. Eight FRs total. Commits `cc43e25` (spec), `47805d8` (red tests, +50 failing), `49eb43b` (impl — co-bundled with spec 08 due to concurrent-agent workdir race; the four spec-07 production files are correct in that commit), `aef6fa6` (docs: ARCHITECTURE.md §1.2/§1.3/§1.6/§2.2/§8.2/§8.3/§8.4 + README invariants). Suite 4025/4025 green. TestFlight scenarios (rapid foreground bursts, reinstall recovery, push-collision) deferred to release smoke. Multi-agent review (R2/R3) deferred — single-agent self-review in R1/R4 walked all eight FRs against the implementation. |
