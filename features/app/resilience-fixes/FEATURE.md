...
| 2026-05-28 | [05-onboarding-defense](specs/05-onboarding-defense/spec.md) | Spec ready for implement. 8 FRs cover (FR1) new `NotContributorError(avatarTypes)` class, (FR2) defensive `/detail` parsing returning `null` on missing assignment+CANDIDATE, (FR3) `/assignments.content` Spring page envelope read with bare-array fallback, (FR4) `fetchAndBuildConfig` throwing `NotContributorError` when both paths fail and **removing** the broken userId-only last-resort branch, (FR5) `useSetup` new `'not-contributor'` step + `nonContributorRoles` state, (FR6) log call to `log.error('onboarding.not-contributor', err, {avatarTypes})` via spec 08, (FR7) new `app/(auth)/not-contributor.tsx` screen with verbatim copy and Sign Out, (FR8) `verifying.tsx` navigation branch. Addresses CROSSOVER_API §15.F5 (pure-manager schema) + §15.F6 (Spring page envelope). |
