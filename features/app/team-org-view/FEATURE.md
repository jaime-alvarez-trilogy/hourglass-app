...
**Status:** Complete (Team tier). Org tier remains flagged off pending unverifiable skip-level roster access.
...
| 2026-07-28 | [04-team-view-content](specs/04-team-view-content/spec.md) | **Complete — feature Team tier done.** `src/hooks/useIsManager.ts` extraction + `TeamViewContent`/`TeamMemberRow`/`getInitials` render branch wired into `app/(tabs)/overview.tsx`. TDD: tests (`fc3f3f4`) → implementation (`87737c4`) → review fixes (`cd07fb5`: privacy leak in `log.error` call, manager-disappears-mid-session fallback, fetchFailed row em-dash+label per spec, photoUrl-failure state keyed on URL not boolean, `useIsManager.ts` JSDoc correction) → test-quality pass (`b6900fe`: converted source-regex/logic-reimplementation tests to real render/real hook execution in `overview.test.tsx`, `approvals.test.tsx`, `useIsManager.test.ts`) → docs (this commit). 259/259 tests passing across the affected suites; full repo suite otherwise green aside from one pre-existing unrelated failure (`NativeWindSmoke.test.tsx`); `tsc --noEmit` clean (only pre-existing repo-wide `fs`/`path`/`__dirname` type-declaration gaps, unrelated to this spec). This closes the `team-org-view` feature's Team tier — all 4 specs complete; Org tier remains flagged off per "Why Org is flagged, not built" above. |
