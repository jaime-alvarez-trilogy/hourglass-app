...
| 2026-06-10 | [03-hourly-pattern-card](specs/03-hourly-pattern-card/spec.md) | Complete. `HourlyPatternCard` component — 24-bar histogram clipped to activeWindow, two-stop AI color gradient (surface→cyan→violet), gold focus overlay + violet AI overlay (suppressed on overlap), two invariant summary text rows. 43 tests passing, 4636 total. |
| 2026-06-10 | [04-overview-integration](specs/04-overview-integration/spec.md) | Complete. Wire `HourlyPatternCard` + `useHourlyInsights()` into `overview.tsx`: imports, stagger count 7→8, hook call, conditional render at index 7 guarded by `profile !== null`. 4653 tests passing. |
